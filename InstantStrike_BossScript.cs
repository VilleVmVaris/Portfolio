...
public class BossScript : MonoBehaviour, Damageable {
    ...
    void Update()
    {
        //Show boss's health when the player approaches and clamp the camera to the arena
    ...
    //This attack creates hazardous areas one by one on the area that the player must avoid
    //The number of safe zones decreases as the boss gets more hurt
    ...
    //Fires the bullets towards the player in a simple pattern
    //The boss moves from one side of the area to the other after it has completed an attack
    // At the moment it has three anchor positions
